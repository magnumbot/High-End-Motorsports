...
export default function App() {
  ...
  React.useEffect(() => {
    const getInventory = async () => {
      ...
      // 1. Define the variety of brands you want in your showroom
      const brands = ['ferrari', 'lamborghini', 'mclaren', 'porsche', 'bugatti'];

      try {
        // 2. Create multiple fetch requests simultaneously
        ...
        // 4. Flatten the arrays (combine all brands into one list)
        const allCars = responses.flat();

        // 5. Map the combined data to your interface
        const formattedData = allCars.map((car: any, index: number) => ({
          id: index + 1,
          make: car.make.toUpperCase(),
          model: car.model.toUpperCase(),
          year: car.year,
          price: Math.floor(Math.random() * (400000 - 200000 + 1)) + 200000,
          hp: Math.floor(Math.random() * (800 - 500 + 1)) + 500
        }));

        setVehicles(formattedData);

      } catch (error) {
        console.error("Showroom fetch failed:", error);
      }
    };

    if (process.env.NEXT_PUBLIC_VEHICLE_API_KEY) {
      getInventory();
    }
  }, []);
  ...
  const renderPage = () => {
    switch (currentPage) {
      case 'home':
        return <HomePage vehicles={vehicles} onExploreClick={() => setCurrentPage('inventory')} />;
      case 'inventory':
